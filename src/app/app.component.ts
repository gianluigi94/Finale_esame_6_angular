...
export class AppComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.deveCaricareImmaginiCarosello = urlIniziale.startsWith('/catalogo'); // Decido se devo caricare le immagini del carosello se parto dal catalogo
    ...
    if (this.deveCaricareImmaginiCarosello)
      // Controllo se devo caricare immagini del carosello
      this.caricamentoCaroselloService.resetta(); // Resetto il servizio del carosello per far partire un caricamento pulito
    ...
    combineLatest([
      ...
      ([
        ...
        try {
          ...
          const path = pulisciUrl(window.location.pathname || ''); // Prendo il path corrente pulito da query e hash
          this.loaderAvvioCatalogo = tipo === 'reload' && path === '/catalogo'; // Segno true se ho ricaricato la pagina mentre ero su /catalogo
        } catch {
