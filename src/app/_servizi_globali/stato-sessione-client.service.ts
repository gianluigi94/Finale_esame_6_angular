...
export class StatoSessioneClientService {
  ...
  constructor() {
  const rawSessione = sessionStorage.getItem('auth');
  const rawLocale = localStorage.getItem('auth');

  const raw = rawSessione ?? rawLocale;
  ...
  if (raw) {
    try {
      ...
      console.warn('[StatoSessioneClientService] errore parse auth da storage', e);
